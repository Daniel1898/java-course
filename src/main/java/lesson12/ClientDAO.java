...
public class ClientDAO {
  ...
  public boolean changePassword(long id, String newPass) throws SQLException {
    Client client = getById(id);
    if (client == null){
      return false;
    }
    if (newPass.length() > 8 && !newPass.contains(client.password)) {
      change(id, client.login, client.age, newPass);
      return true;
    } else {
      return false;
    }
